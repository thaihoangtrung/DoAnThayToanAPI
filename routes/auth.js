...
let avatarDir = path.join(__dirname, '../avatar')
let postcdnURL = `http://localhost:8080/upload_avatar`
/* GET home page. */
...
router.post('/forgotpassword', validatorForgotPassword, validate, async function (req, res, next) {
    try {
        ...
        if (user) {
            ...
            await user.save();
            let url = `http://localhost:3000/reset-password/${user.resetPasswordToken}`
            await sendmail(user.email, "Đặt lại mật khẩu", url)
